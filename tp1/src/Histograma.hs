...
agregar x (Histograma l u ls)
  ...
  | x >= u = Histograma l u (actualizarElem (length ls - 1) (+1) ls)
  | otherwise = Histograma l u (actualizarElem indice (+1) ls)
  where
    nCasilleros = length ls - 2
    tamIntervalo = (u - l) / fromIntegral nCasilleros
    indice = floor ((x - l) / tamIntervalo) + 1
...
casilleros (Histograma inicio tamano cantidades) =
  ...
  where
    ...
    limSup = límitesNormales ++ [1/0]                                                -- Lista de límites superiores

    total = fromIntegral (sum cantidades)                                         -- Total de valores en el Histograma
    calcularPorcentaje c
      | total == 0 = 0
      | otherwise = fromIntegral c / total * 100                                  -- Convierte cantidad a porcentaje
